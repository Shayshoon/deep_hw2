...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        current_dim = in_dim
        for h_dim, non_lin in zip(dims, nonlins):
            self.layers.append(nn.Linear(current_dim, h_dim, bias=True))
            if non_lin in ACTIVATIONS:
                activation = ACTIVATIONS[non_lin](**ACTIVATION_DEFAULT_KWARGS[non_lin])
