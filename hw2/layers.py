...
class TanH(Layer):
    ...
    def forward(self, x, **kw):
        ...
        self.grad_cache = (torch.exp(x)+torch.exp(-x))
        # ========================
    ...
    def backward(self, dout):
        ...
        # TODO: Implement gradient w.r.t. the input x
        # ====== YOUR CODE: ======
        cached = self.grad_cache
        dx = dout * (4 / cached.square()) 
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        s = torch.log(torch.sum(torch.exp(x), dim=1))
        
        scores = x[torch.arange(N), y]
        loss = (s - scores).mean()
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        exp_x = torch.exp(x)
        grad = exp_x / torch.sum(exp_x, dim=1, keepdim=True)
...
class Dropout(Layer):
    ...
    def backward(self, dout):
        ...
        if not self.training_mode:
            ...
            dx = dout * self.mask * scaling_factor
        # ========================
...
class MLP(Layer):
    ...
    def __init__(
        ...
        **kw,
        ...
        for i, in_dim in enumerate(hidden_features):
            layers.append(act())
            if dropout > 0:
                layers.append(Dropout(dropout))
            out_dim = num_classes if i+1 == len(hidden_features) else hidden_features[i+1]
