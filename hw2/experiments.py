...
def mlp_experiment(
    ...
    # ====== YOUR CODE: ======
    in_dim = dl_train.dataset.tensors[0].shape[1]
    
    # Hidden layers + Output layer (2 classes)
    # If depth=2, width=10 -> dims=[10, 10, 2]
    dims = [width] * depth + [2]
    
    # Activations: Tanh for hidden, Identity for output (to keep logits)
    # Note: We avoid Softmax here so CrossEntropyLoss works correctly
    nonlins = ['tanh'] * depth + [torch.nn.Identity()]
    ...
    # 3. Setup Training
    # Use CrossEntropyLoss for the binary classification logits
    ...
    # Extract the full tensors from the validation dataset to run ROC analysis
    ...
    # 6. Evaluate on Test Set with new threshold
    test_result = trainer.test_epoch(dl_test)
    
    # 7. Extract metrics
    valid_acc = fit_result.test_acc[-1] # Accuracy from the last epoch of training
    test_acc = test_result.accuracy     # Accuracy from the specific test run
    # ========================
