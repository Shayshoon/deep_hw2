...
class ArgMaxClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        # ====== YOUR CODE: ======
        return torch.max(y_proba, 1).indices
        # ========================
...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        return (pos_probs >= self.threshold).int()
        # ========================
...
def plot_decision_boundary_2d(
    ...
    x1_min, x1_max = x[:, 0].min() - dx, x[:, 0].max() + dx
    x2_min, x2_max = x[:, 1].min() - dx, x[:, 1].max() + dx

    x_range = torch.arange(x1_min, x1_max, dx)
    y_range = torch.arange(x2_min, x2_max, dx)
    x1_grid, x2_grid = torch.meshgrid(x_range, y_range, indexing='xy')
...
def select_roc_thresh(
    ...
    y_score_np = torch.softmax(y_scores, dim=1)[:, 1]
    y_score_np = y_score_np.detach().numpy()
    y_true_np = y.detach().numpy()

    fpr, tpr, thresh = roc_curve(y_true_np, y_score_np)
    
    optimal_thresh_idx = torch.argmax(torch.from_numpy(tpr - fpr))
    optimal_thresh = thresh[optimal_thresh_idx]
    
    # y_pred = classifier.predict_proba(x).flatten()
    # y
    # fpr, tpr, thresh = roc_curve(y.detach().numpy(), y_pred.detach().numpy())
    
    # J = tpr - fpr
    # optimal_theresh_idx = np.argmax(J)
